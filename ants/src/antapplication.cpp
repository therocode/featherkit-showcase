...
{
    // digger ant! :) //
    size_t diggerAntId = antManager.createAnt(true, true, glm::vec2(320.0f, 1124.0f), 4.5f);
    messageBus.send(DiggerAntCreatedMessage(diggerAntId));
    antManager.createAnt(true, false, glm::vec2(967.0f, 787.0f), (float)(rand() % 25 + 10)/10.0f);
    antManager.createAnt(true, false, glm::vec2(800.0f, 762.0f), (float)(rand() % 25 + 10)/10.0f);
    antManager.createAnt(true, false, glm::vec2(600.0f, 804.0f), (float)(rand() % 25 + 10)/10.0f);
    antManager.createAnt(true, false, glm::vec2(463.0f, 680.0f), (float)(rand() % 25 + 10)/10.0f);
}
